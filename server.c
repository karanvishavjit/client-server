...
char kvSwitchClientMirror(int clientNo);
...
int main(int argc, char *argv[])
{
	...
	{	
		...
		char connectType = kvSwitchClientMirror(client_no);
		
		// Connect 
		if(connectType == KV_CONN_SERVER){
		...
		else if(connectType == KV_CONN_MIRROR) {
			kvClientScoketDes = accept(kvSocketDes, (struct sockaddr *) NULL, NULL);
			printf("Maximm connections reached on Server: Redirection client -> Mirror %d\n", client_no);
			
			// Fork a child process to redirect client
			if (!fork()){	
				redirectToMirror(kvClientScoketDes);
				close(kvClientScoketDes);
				exit(0);
			}
			waitpid(0, &kvConnStatus, WNOHANG);
...
/**
 * 
 * Funcito to switch between mirror ans server
 * first 6 connetion will be served by server next 6 by mirror
 * and after that alternate.
 * */
char kvSwitchClientMirror(int kvClientConn) {

		//if count less than equal to 6 return server
    if (kvClientConn >= 1 && kvClientConn <= 6) {
    
        return KV_CONN_SERVER;
    }
    	//if count less than equal to 6 return mirror
     else if (kvClientConn >= 7 && kvClientConn <= 12) {
        
        return KV_CONN_MIRROR;
    } else {
        	//alternate between mirror and server
        static char kvCurrRoute = KV_CONN_MIRROR;  
        	
        	//if current routing is server swith to mirror vice versa
        kvCurrRoute = (kvCurrRoute == KV_CONN_SERVER) ? KV_CONN_MIRROR : KV_CONN_SERVER;  
...
int redirectToMirror(int kvClientSd){

		//structure to hold ip addr adn port of miror
	kvAddrInfo kvMirrorAddr;
	
	 // Send response type for the mirror struct
    long kvRespType = KV_STR_RES;
    ...
    strcpy(kvMirrorAddr.kv_ipAddr, KV_IP_MIRROR);
    kvMirrorAddr.kv_portNum = KV_PORT_MIRROR;

    // Send mirror IP and Port to client
    if (send(kvClientSd, &kvMirrorAddr, sizeof(kvAddrInfo), 0) == -1) {
        perror("Error sending mirror address");
        return -1;
    }
